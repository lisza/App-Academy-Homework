...
describe Dessert do
  let(:chef) { double("chef", name: "Melissa") }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("brownie", "test") } .to raise_error(ArgumentError)
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Melissa the Great Baker")
      expect(brownie.serve).to eq("Chef Melissa the Great Baker has made 20 brownies!")
    end
  end
